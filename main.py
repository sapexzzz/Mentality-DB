...
class MentalityCore:
    ...
    def load_config(self):
        default = {
            ...
            "search_fields": ["first_name", "last_name", "first_name_en", "last_name_en", "country", "city", "address", "phones"],
            "grid_columns": 5,
            "date_format": "iso",
            "unsaved_action": "ask",
        ...
        if os.path.exists(CONFIG_PATH):
            try:
                ...
                # Migrate legacy search scope to explicit field list.
                legacy_scope = merged.get("search_scope")
                if not isinstance(merged.get("search_fields"), list):
                    merged["search_fields"] = default["search_fields"].copy()
                if isinstance(legacy_scope, str):
                    name_fields = ["first_name", "last_name", "first_name_en", "last_name_en"]
                    other_fields = ["country", "city", "address", "phones"]
                    if legacy_scope == "name":
                        merged["search_fields"] = name_fields
                    elif legacy_scope == "all":
                        merged["search_fields"] = name_fields + other_fields

                # Normalize possibly invalid values from older versions.
                valid_fields = set(default["search_fields"])
                merged["search_fields"] = [f for f in merged.get("search_fields", []) if f in valid_fields]
                if not merged["search_fields"]:
                    merged["search_fields"] = default["search_fields"].copy()
                return merged
            except: pass
        return default
...
class MentalityGUI(ctk.CTk):
    ...
    def setup_global_shortcuts(self):
        # Ctrl+A, Ctrl+Z, Ctrl+Y are not reliably handled by CTkEntry on all
        # Linux Tk builds, so we provide global fallbacks.
        # Ctrl+C/V/X are handled natively by CTkEntry — binding them globally
        # with add="+" would trigger paste/cut/copy twice, so we skip them.
    ...
    def refresh_ui(self):
        ...
        selected_fields = self.core.cfg.get("search_fields", [])
        if not isinstance(selected_fields, list) or not selected_fields:
            selected_fields = ["first_name", "last_name", "first_name_en", "last_name_en", "country", "city", "address", "phones"]
        ...
        for r in rows:
            ...
            haystack = self._normalize_text(" ".join([self._to_text(field_map.get(k, "")) for k in selected_fields]))
    ...
    def open_export_dialog(self):
        ...
        state = {
            "scope": ctk.StringVar(value="full"),
    ...
    def _safe_filename(value):
        """Strip characters unsafe in filenames, limit length."""
    ...
    def perform_export(self, selection=None):
        ...
        if selection and len(selection) == 1:
            with sqlite3.connect(DB_NAME) as conn:
                ...
                fn = self._safe_filename(self._to_text(self.core.decrypt(r[0], "Card")))
                ln = self._safe_filename(self._to_text(self.core.decrypt(r[1], "Data")))
    ...
    def open_settings(self):
        ...
        sw.title(self.loc("settings"))
        sw.geometry("560x640")
        sw.attributes('-topmost', True)

        body = ctk.CTkScrollableFrame(sw, fg_color="transparent")
        body.pack(fill="both", expand=True, padx=14, pady=14)

        ctk.CTkLabel(body, text=self.loc("settings_note"), anchor="w", justify="left").pack(fill="x", pady=(0, 10))

        def apply_search_binding():
        ...
        def set_cfg(key, value, apply_theme=False, refresh=False, update_search=False):
            self.core.cfg[key] = value
            self.core.save_config(self.core.cfg)
            if apply_theme:
                ctk.set_appearance_mode(value)
            if update_search:
                apply_search_binding()
            if refresh:
                self.refresh_ui()

        def change_language(v):
            set_cfg("language", v)
            sw.destroy()
            self.after(50, self.rebuild_ui)

        options = [
            (self.loc("settings_theme"), "theme", ["Dark", "Light"], lambda v: set_cfg("theme", v, apply_theme=True)),
            (self.loc("settings_language"), "language", list(self.core.languages.keys()), change_language),
            (self.loc("settings_search_trigger"), "search_trigger", ["manual", "live"], lambda v: set_cfg("search_trigger", v, update_search=True)),
            (self.loc("settings_grid_columns"), "grid_columns", ["3", "4", "5", "6"], lambda v: set_cfg("grid_columns", int(v), refresh=True)),
        ...
        fields_wrap = ctk.CTkFrame(body)
        fields_wrap.pack(fill="x", pady=(10, 4))
        ctk.CTkLabel(fields_wrap, text=self.loc("settings_search_scope"), anchor="w").pack(anchor="w", padx=10, pady=(8, 6))
        ...
        labels_map = {
            "first_name": self.loc("field_first_name_ru"),
            "last_name": self.loc("field_last_name_ru"),
            "first_name_en": self.loc("field_first_name_en"),
            "last_name_en": self.loc("field_last_name_en"),
            "country": self.loc("field_country"),
            "city": self.loc("field_city"),
            "address": self.loc("field_address"),
            "phones": self.loc("field_phones"),
        }
        field_vars = {}

        def save_search_fields():
            chosen = [k for k, var in field_vars.items() if var.get()]
            if not chosen:
                # at least one field must stay selected
                key = "first_name"
                if key in field_vars:
                    field_vars[key].set(True)
                    chosen = [key]
            self.core.cfg["search_fields"] = chosen
            self.core.save_config(self.core.cfg)
            self.refresh_ui()

        for key in ["first_name", "last_name", "first_name_en", "last_name_en", "country", "city", "address", "phones"]:
            var = ctk.BooleanVar(value=key in selected_fields)
            field_vars[key] = var
            ctk.CTkCheckBox(fields_wrap, text=labels_map[key], variable=var, command=save_search_fields).pack(anchor="w", padx=14, pady=2)
